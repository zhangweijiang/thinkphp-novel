 ...
 * @author Uwe Tews
 * @package Smarty
 * @subpackage __PUBLIC__/Pc/pluginsInternal
...
class Smarty_Internal_Utility {
    ...
    public static function testInstall(Smarty $smarty, &$errors=null)
    {
        ...
        if ($errors === null) {
            echo "Testing __PUBLIC__/Pc/plugins directory...\n";
        }

        // test if all registered __PUBLIC__/Pc/plugins_dir are accessible
        // and if core __PUBLIC__/Pc/plugins directory is still registered
        $_core___PUBLIC__/Pc/plugins_dir = realpath(dirname(__FILE__) .'/../__PUBLIC__/Pc/plugins');
        $_core___PUBLIC__/Pc/plugins_available = false;
        foreach($smarty->get__PUBLIC__/Pc/pluginsDir() as $plugin_dir) {
                ...
                if ($smarty->use_include_path && !preg_match('/^([\/\\\\]|[a-zA-Z]:[\/\\\\])/', $_plugin_dir)) {
                    ...
                    if (($plugin_dir = Smarty_Internal_Get_Include_Path::getIncludePath($_plugin_dir)) !== false) {
                        ...
                        if ($errors === null) {
                            ...
                            $errors['__PUBLIC__/Pc/plugins_dir'] = $message;
                        }
            ...
            } elseif ($_core___PUBLIC__/Pc/plugins_dir && $_core___PUBLIC__/Pc/plugins_dir == realpath($plugin_dir)) {
                $_core___PUBLIC__/Pc/plugins_available = true;
                if ($errors === null) {
                    echo "$plugin_dir is OK.\n";
                }
            } else {
        ...
        if (!$_core___PUBLIC__/Pc/plugins_available) {
            $status = false;
            $message = "WARNING: Smarty's own libs/__PUBLIC__/Pc/plugins is not available";
            if ($errors === null) {
                echo $message . ".\n";
            } elseif (!isset($errors['__PUBLIC__/Pc/plugins_dir'])) {
                $errors['__PUBLIC__/Pc/plugins_dir'] = $message;
            }
        }
        ...
        // test if sys__PUBLIC__/Pc/plugins are available
        $source = SMARTY_SYS__PUBLIC__/Pc/plugins_DIR;
        if (is_dir($source)) {
            $expected = array(
            ...
            if ($expected) {
                $status = false;
                $message = "FAILED: files missing from libs/sys__PUBLIC__/Pc/plugins: ". join(', ', array_keys($expected));
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
                    $errors['sys__PUBLIC__/Pc/plugins'] = $message;
                }
            } elseif ($errors === null) {
                echo "... OK\n";
            }
            ...
            $status = false;
            $message = "FAILED: ". SMARTY_SYS__PUBLIC__/Pc/plugins_DIR .' is not a directory';
            if ($errors === null) {
                echo $message . ".\n";
            } else {
                $errors['sys__PUBLIC__/Pc/plugins_dir_constant'] = $message;
            }
        }
        ...
        // test if core __PUBLIC__/Pc/plugins are available
        $source = SMARTY___PUBLIC__/Pc/plugins_DIR;
        if (is_dir($source)) {
            $expected = array(
            ...
            if ($expected) {
                $status = false;
                $message = "FAILED: files missing from libs/__PUBLIC__/Pc/plugins: ". join(', ', array_keys($expected));
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
                    $errors['__PUBLIC__/Pc/plugins'] = $message;
                }
            } elseif ($errors === null) {
                echo "... OK\n";
            }
            ...
            $status = false;
            $message = "FAILED: ". SMARTY___PUBLIC__/Pc/plugins_DIR .' is not a directory';
            if ($errors === null) {
                echo $message . ".\n";
            } else {
                $errors['__PUBLIC__/Pc/plugins_dir_constant'] = $message;
            }
        }
