...
class Smarty_Internal_Template extends Smarty_Internal_TemplateBase {
    ...
    public $properties = array('file_dependency' => array(),
        ...
        'function' => array());
    /**
     * required __PUBLIC__/Pc/plugins
     * @var array
     */
    public $required___PUBLIC__/Pc/plugins = array('compiled' => array(), 'nocache' => array());
    /**
     ...
     * It load the required template resources and cacher __PUBLIC__/Pc/plugins
     *
    ...
    public function createTemplateCodeFrame($content = '', $cache = false)
    {
        ...
        // include code for __PUBLIC__/Pc/plugins
        if (!$cache) {
            if (!empty($this->required___PUBLIC__/Pc/plugins['compiled'])) {
                $__PUBLIC__/Pc/plugins_string = '<?php ';
                foreach ($this->required___PUBLIC__/Pc/plugins['compiled'] as $tmp) {
                    foreach ($tmp as $data) {
                        $file = addslashes($data['file']);
                        $__PUBLIC__/Pc/plugins_string .= "if (!is_callable('{$data['function']}')) include '{$file}';\n";
                    }
                }
                $__PUBLIC__/Pc/plugins_string .= '?>';
            }
            if (!empty($this->required___PUBLIC__/Pc/plugins['nocache'])) {
                $this->has_nocache_code = true;
                $__PUBLIC__/Pc/plugins_string .= "<?php echo '/*%%SmartyNocache:{$this->properties['nocache_hash']}%%*/<?php \$_smarty = \$_smarty_tpl->smarty; ";
                foreach ($this->required___PUBLIC__/Pc/plugins['nocache'] as $tmp) {
                    foreach ($tmp as $data) {
                        $file = addslashes($data['file']);
                        $__PUBLIC__/Pc/plugins_string .= addslashes("if (!is_callable('{$data['function']}')) include '{$file}';\n");
                    }
                }
                $__PUBLIC__/Pc/plugins_string .= "?>/*/%%SmartyNocache:{$this->properties['nocache_hash']}%%*/';?>\n";
            }
        }
        // build property code
        ...
        $output .= $__PUBLIC__/Pc/plugins_string;
        $output .= $content;
        if (!$this->source->recompiled) {
            $output .= '<?php }} ?>';
        }
        ...
        switch ($property_name) {
            ...
            case 'cached':
                if (!class_exists('Smarty_Template_Cached')) {
                    include SMARTY_SYS__PUBLIC__/Pc/plugins_DIR . 'smarty_cacheresource.php';
                }
