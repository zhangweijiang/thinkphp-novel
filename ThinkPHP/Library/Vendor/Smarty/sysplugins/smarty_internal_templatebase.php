...
abstract class Smarty_Internal_TemplateBase extends Smarty_Internal_Data {
    ...
    public function fetch($template = null, $cache_id = null, $compile_id = null, $parent = null, $display = false, $merge_tpl_vars = true, $no_output_filter = false)
    {
        ...
        if (!($_template->caching == Smarty::CACHING_LIFETIME_CURRENT || $_template->caching == Smarty::CACHING_LIFETIME_SAVED) || !$_template->cached->valid) {
            ...
            if ($_template->parent instanceof Smarty_Internal_Template) {
                ...
                foreach ($_template->required___PUBLIC__/Pc/plugins as $code => $tmp1) {
                    foreach ($tmp1 as $name => $tmp) {
                        foreach ($tmp as $type => $data) {
                            $_template->parent->required___PUBLIC__/Pc/plugins[$code][$name][$type] = $data;
                        }
                    }
                }
            }
    ...
    {
        if (isset($this->smarty->registered___PUBLIC__/Pc/plugins[$type][$tag])) {
            throw new SmartyException("Plugin tag \"{$tag}\" already registered");
        } elseif (!is_callable($callback)) {
            throw new SmartyException("Plugin \"{$tag}\" not callable");
        } else {
            $this->smarty->registered___PUBLIC__/Pc/plugins[$type][$tag] = array($callback, (bool) $cacheable, (array) $cache_attr);
        }
    }
    ...
    {
        if (isset($this->smarty->registered___PUBLIC__/Pc/plugins[$type][$tag])) {
            unset($this->smarty->registered___PUBLIC__/Pc/plugins[$type][$tag]);
        }
    }
