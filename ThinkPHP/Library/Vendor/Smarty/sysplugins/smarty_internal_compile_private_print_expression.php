    ...
    private function compile_output_filter($compiler, $name, $output)
    {
        ...
        if ($path) {
            if ($compiler->template->caching) {
                $compiler->template->required___PUBLIC__/Pc/plugins['nocache'][$name][Smarty::FILTER_VARIABLE]['file'] = $path;
                $compiler->template->required___PUBLIC__/Pc/plugins['nocache'][$name][Smarty::FILTER_VARIABLE]['function'] = $plugin_name;
            } else {
                $compiler->template->required___PUBLIC__/Pc/plugins['compiled'][$name][Smarty::FILTER_VARIABLE]['file'] = $path;
                $compiler->template->required___PUBLIC__/Pc/plugins['compiled'][$name][Smarty::FILTER_VARIABLE]['function'] = $plugin_name;
            }
        } else {
            // not found
            return false;
