...
class Weixinpay {
    ...
    public function __construct($config){
        ...
        if (empty($config)) {
            $set = M('set')->find();
            $this->config['APPID'] = $set['wxappid'];
            $this->config['MCHID'] = $set['wxmchid'];
            $this->config['KEY'] = $set['wxmchkey'];
            $this->config['APPSECRET'] = $set['wxappsecret'];
            $this->config['NOTIFY_URL'] = '/Home/Wxpay/notify/order_number/'; // 接收支付状态的连接
        }else{
            $this->config=$config;
        }
    }
