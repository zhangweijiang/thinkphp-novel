...
class Template_Lite_Compiler extends Template_Lite {
	...
	var $___PUBLIC__/Pc/plugins			=	array();	// stores all internal __PUBLIC__/Pc/plugins
...
//		$this->_func_regexp = '[a-zA-Z_]\w*';
	...
	function _compile_file($file_contents)
	{
		...
		if (count($this->___PUBLIC__/Pc/plugins['prefilter']) > 0)
		{
			foreach ($this->___PUBLIC__/Pc/plugins['prefilter'] as $function)
			{
				if ($function === false)
				{
					continue;
		...
		if (count($this->___PUBLIC__/Pc/plugins['postfilter']) > 0)
		{
			foreach ($this->___PUBLIC__/Pc/plugins['postfilter'] as $function)
			{
				if ($function === false)
				{
					continue;
...
//					$_result .= "['$var']";
	...
	function _plugin_exists($function, $type)
	{
		...
		if (isset($this->___PUBLIC__/Pc/plugins[$type][$function]) && is_array($this->___PUBLIC__/Pc/plugins[$type][$function]) && is_object($this->___PUBLIC__/Pc/plugins[$type][$function][0]) && method_exists($this->___PUBLIC__/Pc/plugins[$type][$function][0], $this->___PUBLIC__/Pc/plugins[$type][$function][1]))
		{
			return '$this->___PUBLIC__/Pc/plugins[\'' . $type . '\'][\'' . $function . '\'][0]->' . $this->___PUBLIC__/Pc/plugins[$type][$function][1];
		}
		// check for standard functions
		if (isset($this->___PUBLIC__/Pc/plugins[$type][$function]) && function_exists($this->___PUBLIC__/Pc/plugins[$type][$function]))
		{
			return $this->___PUBLIC__/Pc/plugins[$type][$function];
		}
	...
	function _load_filters()
	{
		if (count($this->___PUBLIC__/Pc/plugins['prefilter']) > 0)
		{
			foreach ($this->___PUBLIC__/Pc/plugins['prefilter'] as $filter_name => $prefilter)
			{
		...
		if (count($this->___PUBLIC__/Pc/plugins['postfilter']) > 0)
		{
			foreach ($this->___PUBLIC__/Pc/plugins['postfilter'] as $filter_name => $postfilter)
			{
