...
class Template_Lite {
	...
	var $__PUBLIC__/Pc/plugins_dir			= array("__PUBLIC__/Pc/plugins");	// where the __PUBLIC__/Pc/plugins are to be found
	...
	var $___PUBLIC__/Pc/plugins		= array(	   'modifier'	  => array(),
									   'function'	  => array(),
									   'block'		 => array(),
									   'compiler'	  => array(),
									   'resource'	  => array(),
	...
	function load_filter($type, $name)
	{
		switch ($type)
		{
			case 'output':
				...
				$this->___PUBLIC__/Pc/plugins['outputfilter'][$name] = "template_" . $type . "filter_" . $name;
			   break;
			case 'pre':
			case 'post':
				if (!isset($this->___PUBLIC__/Pc/plugins[$type . 'filter'][$name]))
				{
					$this->___PUBLIC__/Pc/plugins[$type . 'filter'][$name] = "template_" . $type . "filter_" . $name;
				}
				break;
		}
	}
	...
	{
		$this->___PUBLIC__/Pc/plugins['modifier'][$modifier] = $implementation;
	}

	function unregister_modifier($modifier)
	{
		unset($this->___PUBLIC__/Pc/plugins['modifier'][$modifier]);
	}

	function register_function($function, $implementation)
	{
		$this->___PUBLIC__/Pc/plugins['function'][$function] = $implementation;
	}

	function unregister_function($function)
	{
		unset($this->___PUBLIC__/Pc/plugins['function'][$function]);
	}

	function register_block($function, $implementation)
	{
		$this->___PUBLIC__/Pc/plugins['block'][$function] = $implementation;
	}

	function unregister_block($function)
	{
		unset($this->___PUBLIC__/Pc/plugins['block'][$function]);
	}

	function register_compiler($function, $implementation)
	{
		$this->___PUBLIC__/Pc/plugins['compiler'][$function] = $implementation;
	}

	function unregister_compiler($function)
	{
		unset($this->___PUBLIC__/Pc/plugins['compiler'][$function]);
	}

	function register_prefilter($function)
	{
		$_name = (is_array($function)) ? $function[1] : $function;
		$this->___PUBLIC__/Pc/plugins['prefilter'][$_name] = $_name;
	}

	function unregister_prefilter($function)
	{
		unset($this->___PUBLIC__/Pc/plugins['prefilter'][$function]);
	}

	function register_postfilter($function)
	{
		$_name = (is_array($function)) ? $function[1] : $function;
		$this->___PUBLIC__/Pc/plugins['postfilter'][$_name] = $_name;
	}

	function unregister_postfilter($function)
	{
		unset($this->___PUBLIC__/Pc/plugins['postfilter'][$function]);
	}

	function register_outputfilter($function)
	{
		$_name = (is_array($function)) ? $function[1] : $function;
		$this->___PUBLIC__/Pc/plugins['outputfilter'][$_name] = $_name;
	}

	function unregister_outputfilter($function)
	{
		unset($this->___PUBLIC__/Pc/plugins['outputfilter'][$function]);
	}
	...
	function register_resource($type, $functions)
	{
		...
		{
			$this->___PUBLIC__/Pc/plugins['resource'][$type] = $functions;
		}
		else
		{
	...
	{
		unset($this->___PUBLIC__/Pc/plugins['resource'][$type]);
	}
	...
	function _get_resource($file)
	{
		...
		else
		{
			...
			$exists = isset($this->___PUBLIC__/Pc/plugins['resource'][$this->_resource_type]) && call_user_func_array($this->___PUBLIC__/Pc/plugins['resource'][$this->_resource_type][1], array($file, &$resource_timestamp, &$this));
...
//		$this->_error_level = error_reporting(E_ALL);
		...
		foreach ($this->___PUBLIC__/Pc/plugins['outputfilter'] as $function)
		{
	...
	function _fetch_compile($file)
	{
		...
		{
			call_user_func_array($this->___PUBLIC__/Pc/plugins['resource'][$this->_resource_type][0], array($file, &$file_contents, &$this));
		}
		...
		$this->_compile_obj->__PUBLIC__/Pc/plugins_dir = &$this->__PUBLIC__/Pc/plugins_dir;
		$this->_compile_obj->template_dir = &$this->template_dir;
		$this->_compile_obj->_vars = &$this->_vars;
		$this->_compile_obj->_confs = &$this->_confs;
		$this->_compile_obj->___PUBLIC__/Pc/plugins = &$this->___PUBLIC__/Pc/plugins;
	...
	function _run_modifier()
	{
		...
		if ($_map_array && is_array($variable))
		{
			foreach($variable as $key => $value)
			{
				...
				{
					$variable[$key] = call_user_func_array($this->___PUBLIC__/Pc/plugins["modifier"][$modifier], $arguments);
				}
			}
		}
		else
		{
			...
			{
				$variable = call_user_func_array($this->___PUBLIC__/Pc/plugins["modifier"][$modifier], $arguments);
			}
		}
		return $variable;
	}
	...
	function _get_plugin_dir($plugin_name)
	{
		...
		$plugin_dir_path = "";
		$_plugin_dir_list = is_array($this->__PUBLIC__/Pc/plugins_dir) ? $this->__PUBLIC__/Pc/plugins_dir : (array)$this->__PUBLIC__/Pc/plugins_dir;
